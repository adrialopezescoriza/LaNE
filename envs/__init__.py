...
def make_env(cfg):
	...
	env = None
	for fn in [make_metaworld_env, make_maniskill_env, make_bigym_env, make_robosuite_env]:
		try:
			env = fn(cfg)
			break
		except ValueError:
			pass
	if env is None:
		raise ValueError(f'Failed to make environment "{cfg.task_name}": please verify that dependencies are installed and that the task exists.')
